...
public class AutoHeightLayout extends ResizeLayout implements ResizeLayout.OnResizeListener {
    ...
    public void setAutoViewHeight(final int height) {
        float heightDp = DisplayUtils.px2dp(mContext, (float) height);
        if (heightDp > 0 && heightDp != mAutoViewHeight) {
            mAutoViewHeight = (int) heightDp;
    ...
    public void showAutoView() {
        if (mAutoHeightLayoutView != null) {
            ...
            setAutoViewHeight(DisplayUtils.dp2px(mContext, mAutoViewHeight));
        }
        mKeyboardState = mKeyboardState == KEYBOARD_STATE_NONE ? KEYBOARD_STATE_FUNC
                : KEYBOARD_STATE_BOTH;
    }
